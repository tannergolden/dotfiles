...
check "manifest records the pre-existing .zshrc as a file" \
  'grep -q "^file	.zshrc	640" "${BACKUP}/manifest.tsv"'
check "manifest records an absent target so restore can delete it" \
  'grep -q "^absent	" "${BACKUP}/manifest.tsv"'

# --- 3. apply --------------------------------------------------------------
export CI_STUB=1
check "apply exits 0" 'cm apply --force'
check "apply overwrote the pre-existing .zshrc (expected, hence the backup)" \
  '! grep -q "PRE-EXISTING USER CONTENT" "${HOME}/.zshrc"'
...
# --- 4. permissions --------------------------------------------------------
# Skipped on Windows: NTFS has ACLs rather than POSIX mode bits, and
# chezmoi's Chmod is a documented no-op there, so asserting a mode would
# fail for a reason that is not a defect in this repository.
case "$(uname -s)" in
...
check "restore put the ORIGINAL .zshrc content back" \
  'grep -q "PRE-EXISTING USER CONTENT" "${HOME}/.zshrc"'
check "restore put the original mode back" \
  '[ "$(stat -c %a "${HOME}/.zshrc" 2>/dev/null || stat -f %Lp "${HOME}/.zshrc")" = "640" ]'
